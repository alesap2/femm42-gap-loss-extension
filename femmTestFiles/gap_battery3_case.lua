-- gap_battery3_case.lua (Lua 4.0 compatible)
-- Battery 3: fringing perpendicular eddy losses study.
-- Two modes: LT0_OFF (baseline) and LT2_ON (perpendicular correction enabled).
-- Sweeps: gap × freq × Bn × d_lam.
-- Reuses gap_battery2_case.lua logic with updated cfg/trace paths.
-- Base .fem is never modified; a patched temporary model is created each run.

cfg_path   = "D:\\FEMM Source\\femmTestFiles\\gap_battery3_case.cfg"
trace_path = "D:\\FEMM Source\\femmTestFiles\\gap_battery3_out\\gap_battery3_trace.log"
...
-- "Amorphous" block    : d_lam only (LamType stays 0).
...
function patch_material_and_save(src_fem, tmp_fem, lam_type, perp_enable, d_lam_str)
  ...
  -- 2. Patch "Amorphous gap" block (re-search after possible length change).
  ...
  -- When PerpLenz is enabled, copy Sigma → Sigma_t so the Bessel model
  -- uses the correct conductivity for the perpendicular direction.
...
-- B.n mean via line integral + midpoint Bx/By diagnostics.
...
-- Tune current to achieve target_bn_t (N=8, S=14×35 mm²).
local N_turns = 8
...
bn_mean, bx_h_mean, by_h_mean = mean_abs_bn_line(h_x0, h_y0, h_x1, h_y1, 0)
...
-- Core region losses (all amorphous blocks + gap blocks).
...
-- Side-gap blocks only (inner-leg pieces adjacent to gap faces).
mo_selectblock(4.4, 17.15)
mo_selectblock(4.4, 53.45 + gap_extra)
P_side = mo_blockintegral(6)
mo_clearblock()

-- ── Extra: Bx/By at gap face (y = 34.3 mm, gap lower face) and at y=14mm ─────
-- These two reference points characterise the fringing-flux amplitude:
--   y_gap_face = 34.3 mm  (lower face of the gap, highest Bx from fringing)
--   y_leg_base  = 14.5 mm  (bottom of inner leg, low fringing)
local y_gap_face = 34.3
local A1, Bx_gf, By_gf = mo_getpointvalues(7.0, y_gap_face)
local A2, Bx_lb, By_lb = mo_getpointvalues(7.0, 14.5)
if Bx_gf == nil then Bx_gf = 0 end
if By_gf == nil then By_gf = 0 end
if Bx_lb == nil then Bx_lb = 0 end
if By_lb == nil then By_lb = 0 end
-- Use y=24mm (existing hline midpoint) for continuity with battery1/2.
-- Also sample y=34.3mm (gap face) and y=14.5mm (leg base).
...
--          bx_gapface_t, by_gapface_t,   <- new: at y=34.3mm
--          l_h, z_ohm, p_tot_w, p_core_w, p_side_w

summary_line = format(
  "%s,%s,%.6f,%d,%.6f,%s,%d,%d,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e",
...
-- ── Vertical Bx/By profile (x=0, y=v_y0..v_y1) ──────────────────────────────
-- Profiles the full inner-leg + gap region to map Bx(y) fringing distribution.
-- Points at x=0 may show Bx≈0 by symmetry; use x=7mm (leg centre) instead.
local prof_x = 7.0
local i = 0
while i < v_n do
  local t = 0
  ...
  local A, Bx, By = mo_getpointvalues(prof_x, y)
  if Bx == nil then Bx = 0 end
  if By == nil then By = 0 end
