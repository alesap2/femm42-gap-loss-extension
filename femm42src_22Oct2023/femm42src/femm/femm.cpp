...
#ifdef MATHLINK
	if (__argc>1)
		if(strcmp("-mathlink",__argv[1])==0) bMathLink=TRUE;
...
//				StatBar->SetPaneText(0,"Ready",TRUE);
		...
		if (time(NULL)!=timestamp) Sleep(1);
		CWinApp::OnIdle(0);
		return TRUE;
	}
#endif
...
int CFemmApp::lua_promptbox(lua_State *L)
{
	...
	if(lua_gettop(L)>0) dlg.mytitle = lua_tostring(L,1);
	dlg.DoModal();
...
int CFemmApp::lua_ERROR(lua_State *L)
{
	...
	else if (!theApp.bActiveX) AfxMessageBox(errmsg);
...
{
	if(lua_gettop(L)>0) theApp.CreateNewDocument((int) lua_todouble(L,1));
	return 0;
}
...
void CFemmApp::CreateNewDocument(int n)
{
	...
	if(pTemp!=NULL) pTemp->OpenDocumentFile(NULL);
}
...
void CFemmApp::OnFileNew() 
{
	...
	if(dlg.DoModal()==IDOK)	CreateNewDocument(dlg.doctype);
}
...
{
	if (((CFemmApp *)AfxGetApp())->bMathLink==FALSE) return 0;
	...
	for(int k=1;k<=n;k++)
	{
		...
		if(lua_type(L,k)==LUA_TNUMBER){
			...
			if (Im(z)==0) MLPutReal(alink,Re(z));
			else{
...
int CFemmApp::lua_open_mathlink(lua_State *L)
{
	if (((CFemmApp *)AfxGetApp())->bMathLink==FALSE)
	{
		mathenv = MLInitialize(0);
		alink = MLOpen(__argc,__argv);
		if(alink==NULL) MsgBox("problem opening a MathLink link\n");
		else ((CFemmApp *)AfxGetApp())->bMathLink=TRUE; 
	}
...
BOOL CFemmApp::EOCheckIfAlreadyOpen(CString p)
{
	...
	// get pointer to document template for FemmviewDoc
	tpos=AfxGetApp()->GetFirstDocTemplatePosition( );	
	for(int i=0;i<6;i++) pTemp=AfxGetApp()->GetNextDocTemplate(tpos);
...
BOOL CFemmApp::HOCheckIfAlreadyOpen(CString p)
{
	...
	// get pointer to document template for FemmviewDoc
	tpos=AfxGetApp()->GetFirstDocTemplatePosition( );	
	for(int i=0;i<7;i++) pTemp=AfxGetApp()->GetNextDocTemplate(tpos);
...
BOOL CFemmApp::COCheckIfAlreadyOpen(CString p)
{
	...
	// get pointer to document template for FemmviewDoc
	tpos=AfxGetApp()->GetFirstDocTemplatePosition( );	
	for(int i=0;i<8;i++) pTemp=AfxGetApp()->GetNextDocTemplate(tpos);
...
int CFemmApp::lua_to_filelink(lua_State *L)
{
	...
	if(n==0) LuaResult="[]\n";
		...
		{
			s=lua_tostring(L,k);
			if(k!=n) LuaResult = LuaResult + s + ",";
			else LuaResult = LuaResult + s + "]\n";
		}
	}
...
{
	if (lua_gettop(L)!=0) ((CFemmApp*) AfxGetApp())->CompatibilityMode=(int) lua_todouble(L,1);
...
int CFemmApp::lua_smartmesh(lua_State *L)
{
	...
	if (lua_gettop(L)!=0) ((CFemmApp*) AfxGetApp())->session_SmartMesh = (int)lua_todouble(L,1);
