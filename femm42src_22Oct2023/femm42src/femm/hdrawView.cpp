...
IMPLEMENT_DYNCREATE(ChdrawView, CView)
...
void ChdrawView::ScreenToDwg(int xs, int ys, double *xd, double *yd, RECT *r)
{
	double x,y;
	x=(double) xs;
	y=(double) ys;
...
BOOL ChdrawView::DwgToScreen(double xd, double yd, int *xs, int *ys, RECT *r)
{
	double x,y;
	x=mag*(xd-ox);
...
void ChdrawView::OnInitialUpdate()
{
...
BOOL ChdrawView::OnPreparePrinting(CPrintInfo* pInfo)
{
	// default preparation
	return DoPreparePrinting(pInfo);
}

void ChdrawView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: add extra initialization before printing
}

void ChdrawView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
...
BOOL ChdrawView::GetBoundingBox(double *x, double *y)
{
...
void ChdrawView::OnBDSelectCirc() 
{
	SelectCircFlag=TRUE;
	SelectWndFlag=FALSE;
	ZoomWndFlag=FALSE;
...
void ChdrawView::OnEditCopyAsMetafile() 
{
...
void ChdrawView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
...
void ChdrawView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void ChdrawView::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
		...
		for(k=0;k<TheDoc->blocklist.GetSize();k++)
		{
			if(TheDoc->blocklist[k].IsExternal){
				...
				for(i=0;i<TheDoc->blockproplist.GetSize();i++)
					...
					{
						if(TheDoc->blockproplist[i].Kx!=TheDoc->blockproplist[i].Ky) bFlag=TRUE;
					}
			}
		}
		if(bFlag)
...
void ChdrawView::OnMenuViewres() 
{
	...
	CString pn = TheDoc->GetPathName();
	if (pn.GetLength()==0){
		MsgBox("No results to display",MB_OK | MB_ICONEXCLAMATION,0);
		return;
	}
	
	CString rootname= "\"" + pn.Left(pn.ReverseFind('.'))+".anh\"";
	((CFemmApp *) AfxGetApp())->HOCheckIfAlreadyOpen(rootname);
	AfxGetApp( )->OpenDocumentFile(rootname);
}

void ChdrawView::OnUndo() 
{
	ChdrawDoc *TheDoc = GetDocument();
	TheDoc->Undo();
	TheDoc->UnselectAll();
...
BOOL ChdrawView::OnEraseBkgnd(CDC* pDC) 
{
...
void ChdrawView::OnOpenSelected() 
{
	ChdrawDoc *pDoc=GetDocument();
...
BOOL ChdrawView::ScanPreferences()
{
	FILE *fp;
	CString fname;

	fname=BinDir+"hdraw.cfg";
...
BOOL ChdrawView::WritePreferences()
{	
	FILE *fp;
	CString fname;
	ChdrawDoc *pDoc=GetDocument();
...
void ChdrawView::OnViewShownames() 
{
	// TODO: Add your command handler code here
...
void ChdrawView::MyMoveTo(CDC *pDC, int x, int y)
{
	Xm=x;
	Ym=y;
}

void ChdrawView::MyLineTo(CDC *pDC, int x, int y)
{
	// trivial case
	if ((x==Xm) && (y==Ym)) return;
...
void ChdrawView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView) 
{
...
BOOL ChdrawView::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) 
{
...
void ChdrawView::OnViewShowOrphans() 
{
	ChdrawDoc *pDoc=GetDocument();
...
void ChdrawView::OnUpdateEditExterior(CCmdUI* pCmdUI) 
{
	if(GetDocument()->ProblemType) pCmdUI->Enable ( TRUE);
	else pCmdUI->Enable ( FALSE );
}

void ChdrawView::OnCreateRadius()
{
	SelectWndFlag=FALSE;
	ZoomWndFlag=FALSE;
	SelectCircFlag=FALSE;
