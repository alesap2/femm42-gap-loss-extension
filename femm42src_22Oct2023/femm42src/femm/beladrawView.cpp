// beladrawView.cpp : implementation of the CbeladrawView class
//
...
IMPLEMENT_DYNCREATE(CbeladrawView, CView)

BEGIN_MESSAGE_MAP(CbeladrawView, CView)
	//{{AFX_MSG_MAP(CbeladrawView)
	...
	ON_COMMAND(ID_SELECTCIRC, OnBDSelectCirc)
	ON_WM_ERASEBKGND()
...
/////////////////////////////////////////////////////////////////////////////
// CbeladrawView construction/destruction

CbeladrawView::CbeladrawView()
{
...
void CbeladrawView::OnNewDocument()
{
	EditAction=d_action;
	ox=0.; oy=0.;
	mag=d_mag;	
...
BOOL CbeladrawView::PreCreateWindow(CREATESTRUCT& cs)
{
...
/////////////////////////////////////////////////////////////////////////////
// CbeladrawView drawing

void CbeladrawView::ScreenToDwg(int xs, int ys, double *xd, double *yd, RECT *r)
{
	double x,y;
	x=(double) xs;
	y=(double) ys;
...
BOOL CbeladrawView::DwgToScreen(double xd, double yd, int *xs, int *ys, RECT *r)
{
	double x,y;
	x=mag*(xd-ox);
...
void CbeladrawView::OnDraw(CDC* pDC)
{
	CbeladrawDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
void CbeladrawView::OnInitialUpdate()
{
...
/////////////////////////////////////////////////////////////////////////////
// CbeladrawView printing

BOOL CbeladrawView::OnPreparePrinting(CPrintInfo* pInfo)
{
	// default preparation
	return DoPreparePrinting(pInfo);
}

void CbeladrawView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: add extra initialization before printing
}

void CbeladrawView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: add cleanup after printing
}

/////////////////////////////////////////////////////////////////////////////
// CbeladrawView diagnostics

#ifdef _DEBUG
void CbeladrawView::AssertValid() const
{
	CView::AssertValid();
}

void CbeladrawView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}

CbeladrawDoc* CbeladrawView::GetDocument() // non-debug version is inline
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CbeladrawDoc)));
	return (CbeladrawDoc*)m_pDocument;
}
#endif //_DEBUG

/////////////////////////////////////////////////////////////////////////////
// CbeladrawView message handlers

void CbeladrawView::OnNodeOp() 
{
...
void CbeladrawView::OnSegmentOp() 
{
	// TODO: Add your command handler code here
	CbeladrawDoc *pDoc;
	pDoc=GetDocument();
...
void CbeladrawView::OnMouseMove(UINT nFlags, CPoint point) 
{
...
BOOL CbeladrawView::GetBoundingBox(double *x, double *y)
{
	...
	if (pDoc->nodelist.GetSize()<2) return FALSE;
...
void CbeladrawView::OnZoomNatural() 
{
	// TODO: Add your command handler code here
	CbeladrawDoc *pDoc=GetDocument();
	...
	if (GetBoundingBox(x,y)==FALSE) return;
...
void CbeladrawView::OnBDSelectCirc() 
{
	SelectCircFlag=TRUE;
	SelectWndFlag=FALSE;
	ZoomWndFlag=FALSE;
	...
	CToolBarCtrl *tc=&pToolBar->GetToolBarCtrl();
	tc->PressButton(ID_SELECTCIRC,TRUE);
}

void CbeladrawView::OnZoomWnd() 
{
	// TODO: Add your command handler code here
	ZoomWndFlag=TRUE;
	SelectWndFlag=FALSE;
...
void CbeladrawView::OnSelectwnd() 
{
	// TODO: Add your command handler code here
	SelectWndFlag=TRUE;
	SelectCircFlag=FALSE;
...
void CbeladrawView::OnEditCopyAsMetafile() 
{
...
void CbeladrawView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if (nChar==VK_F3)
	{
		...
		else DefaultMeshSize=0;
		for (i=0;i<pDoc->blocklist.GetSize();i++)
		{
			if (pDoc->blocklist[i].MaxArea>0) pDoc->blocklist[i].MaxArea/=4.;
			else if (pDoc->SmartMesh) pDoc->blocklist[i].MaxArea=DefaultMeshSize/4.;	
		}
		InvalidateRect(NULL);
	}
...
void CbeladrawView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if(bLinehook!=FALSE){
...
void CbeladrawView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if(bLinehook!=FALSE){
	...
	if (SelectCircFlag==2){
		...
		CToolBar *pToolBar=&MFrm->m_leftbar;    
		CToolBarCtrl *tc=&pToolBar->GetToolBarCtrl();
		tc->PressButton(ID_SELECTCIRC,FALSE);
...
void CbeladrawView::OnRButtonDown(UINT nFlags, CPoint point) 
{
	if(bLinehook!=FALSE){
...
void CbeladrawView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void CbeladrawView::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if (EditAction==0)
	{
		...
		if(i>=0){
			...
			sprintf(s,"In Group:  %i\n",pDoc->nodelist[i].InGroup);
			ss+=s;
...
void CbeladrawView::OnMakeMesh() 
{
	CbeladrawDoc *TheDoc = GetDocument();
	...
	if(LoadMesh==TRUE){
		...
		else lua_pushnumber(lua,(int) TheDoc->meshnode.GetSize());
	}
	
}

void CbeladrawView::OnMenuAnalyze() 
{
	...
	if(TheDoc->ProblemType==1)
	{
		...
		for(k=0;k<TheDoc->blocklist.GetSize();k++)
		{
			if(TheDoc->blocklist[k].IsExternal){
				if ((TheDoc->extRi==0) || (TheDoc->extRo==0)) bExteriorDefined=FALSE;
				for(i=0;i<TheDoc->blockproplist.GetSize();i++)
					if(TheDoc->blocklist[k].BlockType==TheDoc->blockproplist[i].BlockName)
					{
						if(TheDoc->blockproplist[i].ex!=TheDoc->blockproplist[i].ey) bFlag=TRUE;
					}
			}
		}
		if(bFlag)
		{
			InvalidateRect(NULL);
			CString ermsg="Only istropic materials are allowed\n";
...
void CbeladrawView::OnMenuViewres() 
{
	// TODO: Add your command handler code here
	CbeladrawDoc *TheDoc = GetDocument();
	...
	((CFemmApp *) AfxGetApp())->EOCheckIfAlreadyOpen(rootname);
	AfxGetApp( )->OpenDocumentFile(rootname);

}

void CbeladrawView::OnUndo() 
{
	CbeladrawDoc *TheDoc = GetDocument();
	TheDoc->Undo();
	TheDoc->UnselectAll();
...
void CbeladrawView::OnMoveObjects() 
{
	// TODO: Add your command handler code here
	CCopyDlg dlg;
	CbeladrawDoc *pDoc=GetDocument();
...
void CbeladrawView::OnCopyObjects() 
{
	// TODO: Add your command handler code here
	CCopyDlg dlg;
	CbeladrawDoc *pDoc=GetDocument();
...
void CbeladrawView::OnPurgemesh() 
{
	// TODO: Add your command handler code here
	CbeladrawDoc *pDoc=GetDocument();
	pDoc->meshnode.RemoveAll();
...
BOOL CbeladrawView::OnEraseBkgnd(CDC* pDC) 
{
...
void CbeladrawView::OnMenuMatlib() 
{
	bd_CLibDlg dlg;
...
void CbeladrawView::OnOpenSelected() 
{
	CbeladrawDoc *pDoc=GetDocument();

	if(EditAction==1) pDoc->OpSegDlg();
	if(EditAction==2){
	...
	if(EditAction==0) pDoc->OpNodeDlg();
	
	if(EditAction==4) pDoc->OpGrpDlg();

	pDoc->UnselectAll();
	DrawPSLG();
}

void CbeladrawView::OnEditScale() 
{
	// TODO: Add your command handler code here
	CScaleDlg dlg;
	CbeladrawDoc *pDoc=GetDocument();
	...
	for(i=0;i<pDoc->nodelist.GetSize();i++)
		if(pDoc->nodelist[i].IsSelected==TRUE){
			if(bFlag==FALSE){
				bFlag=TRUE;
				x[0]=pDoc->nodelist[i].x; x[1]=x[0];
				y[0]=pDoc->nodelist[i].y; y[1]=y[0];	
			}
			if(pDoc->nodelist[i].x<x[0]) x[0]=pDoc->nodelist[i].x;
			if(pDoc->nodelist[i].x>x[1]) x[1]=pDoc->nodelist[i].x;
			if(pDoc->nodelist[i].y<y[0]) y[0]=pDoc->nodelist[i].y;
	...
	for(i=0;i<pDoc->blocklist.GetSize();i++)
		if(pDoc->blocklist[i].IsSelected==TRUE){
			if(bFlag==FALSE){
				bFlag=TRUE;
				x[0]=pDoc->blocklist[i].x; x[1]=x[0];
				y[0]=pDoc->blocklist[i].y; y[1]=y[0];	
			}
			if(pDoc->blocklist[i].x<x[0]) x[0]=pDoc->blocklist[i].x;
			if(pDoc->blocklist[i].x>x[1]) x[1]=pDoc->blocklist[i].x;
			if(pDoc->blocklist[i].y<y[0]) y[0]=pDoc->blocklist[i].y;
	...
	for(i=0;i<pDoc->linelist.GetSize();i++)
		if(pDoc->linelist[i].IsSelected==TRUE){
			for(j=0;j<2;j++){
				if(j==0) k=pDoc->linelist[i].n0;
				else k=pDoc->linelist[i].n1;
				if(bFlag==FALSE){
					bFlag=TRUE;
					x[0]=pDoc->nodelist[k].x; x[1]=x[0];
					y[0]=pDoc->nodelist[k].y; y[1]=y[0];	
				}
				if(pDoc->nodelist[k].x<x[0]) x[0]=pDoc->nodelist[k].x;
				if(pDoc->nodelist[k].x>x[1]) x[1]=pDoc->nodelist[k].x;
				if(pDoc->nodelist[k].y<y[0]) y[0]=pDoc->nodelist[k].y;
				if(pDoc->nodelist[k].y>y[1]) y[1]=pDoc->nodelist[k].y;
			}
		}
...
void CbeladrawView::OnEditMirror() 
{
	// TODO: Add your command handler code here
	CMirrorDlg dlg;
	CbeladrawDoc *pDoc=GetDocument();
...
void CbeladrawView::OnEditCut() 
{
	CbeladrawDoc *pDoc=GetDocument();
	BOOL flag;

	pDoc->UpdateUndo();
	if(EditAction==0) flag=pDoc->DeleteSelectedNodes();
	if(EditAction==1) flag=pDoc->DeleteSelectedSegments();
	if(EditAction==2) flag=pDoc->DeleteSelectedBlockLabels();
	if(EditAction==3) flag=pDoc->DeleteSelectedArcSegments();
	if(EditAction==4){
		flag=FALSE;
		if (pDoc->DeleteSelectedSegments()==TRUE) flag=TRUE;
		if (pDoc->DeleteSelectedArcSegments()==TRUE) flag=TRUE;
		if (pDoc->DeleteSelectedNodes()==TRUE) flag=TRUE;
		if (pDoc->DeleteSelectedBlockLabels()==TRUE) flag=TRUE;
	}
	if (flag==TRUE){
		MeshUpToDate=FALSE;
		if(MeshFlag==TRUE) OnShowMesh();
...
BOOL CbeladrawView::ScanPreferences()
{
	FILE *fp;
	CString fname;
...
BOOL CbeladrawView::WritePreferences()
{	
	FILE *fp;
	CString fname;
	CbeladrawDoc *pDoc=GetDocument();
...
void CbeladrawView::OnViewShownames() 
{
	// TODO: Add your command handler code here
...
void CbeladrawView::MyMoveTo(CDC *pDC, int x, int y)
{
	Xm=x;
	Ym=y;
}

void CbeladrawView::MyLineTo(CDC *pDC, int x, int y)
{
	// trivial case
	if ((x==Xm) && (y==Ym)) return;
...
void CbeladrawView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView) 
{
...
BOOL CbeladrawView::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) 
{
...
void CbeladrawView::OnViewShowOrphans() 
{
	CbeladrawDoc *pDoc=GetDocument();
...
void CbeladrawView::OnUpdateEditExterior(CCmdUI* pCmdUI) 
{
	if(GetDocument()->ProblemType) pCmdUI->Enable ( TRUE);
	else pCmdUI->Enable ( FALSE );
}

void CbeladrawView::OnCreateRadius()
{
	SelectWndFlag=FALSE;
	ZoomWndFlag=FALSE;
	SelectCircFlag=FALSE;
...
void CbeladrawView::OnMakeABC()
{
	CMakeABCDlg dlg;
	CbeladrawDoc *pDoc=GetDocument();

	double xx[2],yy[2];
	if (GetBoundingBox(xx,yy)==0) return;
	...
	if(dlg.DoModal()==IDOK){
		pDoc->UpdateUndo();
		CString LuaCmd;
		LuaCmd.Format("ei_makeABC(%i,%g,%g,%g,%i)",dlg.abcn,dlg.abcr,dlg.abcx,dlg.abcy,dlg.n);
