// cdrawView.cpp : implementation of the CcdrawView class
//
...
IMPLEMENT_DYNCREATE(CcdrawView, CView)

BEGIN_MESSAGE_MAP(CcdrawView, CView)
	//{{AFX_MSG_MAP(CcdrawView)
...
/////////////////////////////////////////////////////////////////////////////
// CcdrawView construction/destruction

CcdrawView::CcdrawView()
{
...
BOOL CcdrawView::PreCreateWindow(CREATESTRUCT& cs)
{
...
void CcdrawView::ScreenToDwg(int xs, int ys, double *xd, double *yd, RECT *r)
{
	double x,y;
	x=(double) xs;
	y=(double) ys;
...
BOOL CcdrawView::DwgToScreen(double xd, double yd, int *xs, int *ys, RECT *r)
{
	double x,y;
	x=mag*(xd-ox);
...
BOOL CcdrawView::OnPreparePrinting(CPrintInfo* pInfo)
{
	// default preparation
	return DoPreparePrinting(pInfo);
}

void CcdrawView::OnBeginPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: add extra initialization before printing
}

void CcdrawView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	// TODO: add cleanup after printing
}

/////////////////////////////////////////////////////////////////////////////
// CcdrawView diagnostics

#ifdef _DEBUG
void CcdrawView::AssertValid() const
{
	CView::AssertValid();
}

void CcdrawView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}

CcdrawDoc* CcdrawView::GetDocument() // non-debug version is inline
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CcdrawDoc)));
	return (CcdrawDoc*)m_pDocument;
}
#endif //_DEBUG

/////////////////////////////////////////////////////////////////////////////
// CcdrawView message handlers

void CcdrawView::OnNodeOp() 
{
...
void CcdrawView::OnMouseMove(UINT nFlags, CPoint point) 
{
...
BOOL CcdrawView::GetBoundingBox(double *x, double *y)
{
...
void CcdrawView::OnBDSelectCirc() 
{
	SelectCircFlag=TRUE;
	SelectWndFlag=FALSE;
	ZoomWndFlag=FALSE;
...
void CcdrawView::OnEditCopyAsMetafile() 
{
...
void CcdrawView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
...
void CcdrawView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if(bLinehook!=FALSE){
...
void CcdrawView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if(bLinehook!=FALSE){
...
void CcdrawView::OnRButtonDown(UINT nFlags, CPoint point) 
{
	if(bLinehook!=FALSE){
...
void CcdrawView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
...
void CcdrawView::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CcdrawView::OnMakeMesh() 
{
	CcdrawDoc *TheDoc = GetDocument();
		...
		for(k=0;k<TheDoc->blocklist.GetSize();k++)
		{
			if(TheDoc->blocklist[k].IsExternal){
				...
				for(i=0;i<TheDoc->blockproplist.GetSize();i++)
					...
					{
						if(TheDoc->blockproplist[i].ox!=TheDoc->blockproplist[i].oy) bFlag=TRUE;
						if(TheDoc->blockproplist[i].ex!=TheDoc->blockproplist[i].ey) bFlag=TRUE;
						if(TheDoc->blockproplist[i].ltx!=TheDoc->blockproplist[i].lty) bFlag=TRUE;
					}
			}
		}
		if(bFlag)
...
void CcdrawView::OnMenuViewres() 
{
	...
	((CFemmApp *) AfxGetApp())->COCheckIfAlreadyOpen(rootname);
	AfxGetApp( )->OpenDocumentFile(rootname);

}

void CcdrawView::OnUndo() 
{
	CcdrawDoc *TheDoc = GetDocument();
	TheDoc->Undo();
	TheDoc->UnselectAll();
...
BOOL CcdrawView::OnEraseBkgnd(CDC* pDC) 
{
...
void CcdrawView::OnOpenSelected() 
{
	CcdrawDoc *pDoc=GetDocument();
...
BOOL CcdrawView::ScanPreferences()
{
	FILE *fp;
	CString fname;

	fname=BinDir+"cdraw.cfg";
...
BOOL CcdrawView::WritePreferences()
{	
	FILE *fp;
	CString fname;
	CcdrawDoc *pDoc=GetDocument();
...
void CcdrawView::OnViewShownames() 
{
	// TODO: Add your command handler code here
...
void CcdrawView::MyMoveTo(CDC *pDC, int x, int y)
{
	Xm=x;
	Ym=y;
}

void CcdrawView::MyLineTo(CDC *pDC, int x, int y)
{
	// trivial case
	if ((x==Xm) && (y==Ym)) return;
...
void CcdrawView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView) 
{
...
BOOL CcdrawView::OnCmdMsg(UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) 
{
...
void CcdrawView::OnViewShowOrphans() 
{
	CcdrawDoc *pDoc=GetDocument();
...
void CcdrawView::OnUpdateEditExterior(CCmdUI* pCmdUI) 
{
	if(GetDocument()->ProblemType) pCmdUI->Enable ( TRUE);
	else pCmdUI->Enable ( FALSE );
}

void CcdrawView::OnCreateRadius()
{
	SelectWndFlag=FALSE;
	ZoomWndFlag=FALSE;
	SelectCircFlag=FALSE;
...
void CcdrawView::OnMakeABC()
{
	...
	if(dlg.DoModal()==IDOK){
		pDoc->UpdateUndo();
		CString LuaCmd;
		LuaCmd.Format("ci_makeABC(%i,%g,%g,%g,%i)",dlg.abcn,dlg.abcr,dlg.abcx,dlg.abcy,dlg.n);
