...
#define CatchNullDocument(); if (pBeladrawDoc==NULL)\
...
int CbeladrawDoc::lua_switchfocus(lua_State *L)
{
	...
	for (int i=0;i<2;i++) pTemp=AfxGetApp()->GetNextDocTemplate(tpos);
...
{
	CatchNullDocument();
	CbeladrawDoc * thisDoc;CbeladrawView * theView;
