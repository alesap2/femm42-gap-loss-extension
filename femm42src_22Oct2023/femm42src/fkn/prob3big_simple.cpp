...
do{
	...
	for(i=0;i<NumEls;i++){
		...
		a_hat=fabs(a_hat); // needed for a special case where the elements are poorly shaped enough to change the ordering of the nodes to maintain CCW numbering
		vol=2.*R*a_hat;
		...
		// rather than just setting these entries to 1 or something....
		for(j=0;j<3;j++)
			if (rn[j]<1.e-06) My[j][j]+=My[0][0]+My[1][1]+My[2][2];
		...
		for (j = 0;j<3;j++)
			for (k = j;k<3;k++)
				Mxy[j][k] += K*(p[j]*rn[j]*q[k]*rn[k] + q[j]*rn[j]*p[k]*rn[k])/2.;
...
/*		
		...
		for (k=0,cc=0;k<3;k++)
			cc += Mx[k][k] + My[k][k];
		if (cc>0) MsgBox("Negative Trace!\n(%g,%g)\n(%g,%g)\n(%g,%g)",meshnode[n[0]].x,meshnode[n[0]].y,meshnode[n[1]].x,meshnode[n[1]].y,meshnode[n[2]].x,meshnode[n[2]].y);
*/
		for (j=0;j<3;j++){
			for (k=j;k<3;k++)
